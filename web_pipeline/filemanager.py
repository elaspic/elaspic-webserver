...
class FileManager:
    ...
    def _make_row(self, m):
        ...
        mutCompleted = (
            m.mut.status == "done"
            and m.mut.affectedType != "NO"
            and (self.job.isDone or not m.mut.rerun)
        )
        c = 1
        if mutCompleted:
            rm = m.realMut[0]
        ...
        if self.file_name == "allresults":
            ...
            bodyline += (
                ...
                if inInterface and mutCompleted
                else r(6)
            )
        ...
        if self.file_name == "allresults":
            ...
            header += (
                ["Model/DOPE_score", "Provean_score", "Matrix_score"]
                + wtmut(["Secondary_structure", "Solvent_accessibility"])
            ...
            bodyline += (
                [
                    ...
                    (
                        ...
                        if rm.secondary_structure_wt in sDict
                        else "-"
                    ),
                    (
                        sDict[rm.secondary_structure_mut]
                        if rm.secondary_structure_mut in sDict
                        else "-"
                    ),
                ...
                + rm.physchem_mut_ownchain.split(",")
                if mutCompleted
                else r(23)
            )

            header += (
                [n[0] + "_wt" for n in elaspic_snippets._names_stability]
                + [n[0] + "_mut" for n in elaspic_snippets._names_stability]
                + wtmut(["IntraclashesEnergy1", "IntraclashesEnergy2"])
                + [
    ...
    def add_alignment_files(self):
        for m in self.muts:
            ...
            if isinstance(rm.model, models._CoreModel):
                ...
                fname = "{}_{} (with {}_{}).aln".format(
                    m.inputIdentifier,
                    defs,
                    inacprot,
                    inacdefs,
                )
                fname2 = "{}_{} (with {}_{}).aln".format(
                    inacprot,
                    inacdefs,
                    m.inputIdentifier,
                    defs,
